...
# add: FullyConn_NN输入是174维，FCNN_CNN1D输入是168维
class FullyConn_NN(torch.nn.Module):
    ...
    def forward(self, x):
        out_c = self.classify(x)#分类
        out_r = self.regress(x)#回归
        return out_c, out_r


class FCNN_CNN1D(torch.nn.Module): #卷积
    def __init__(self):
    ...
    def forward(self, x):# 68  + 80
        lane_fea = x[:, -80:]#lane feature  后80
        lane_fea = lane_fea.view(lane_fea.size(0), 4, 20)

        obs_fea = x[:, :-80]#obstacle feature 前68

        lane_fea = self.lane_feature_conv(lane_fea) #lane_fea conv1d

        lane_fea_max = self.lane_feature_maxpool(lane_fea) #lane_fea  maxpool
        lane_fea_avg = self.lane_feature_avgpool(lane_fea) #lane_fea  avgpool

        lane_fea = torch.cat([lane_fea_max.view(lane_fea_max.size(0), -1),
                              lane_fea_avg.view(lane_fea_avg.size(0), -1)], 1) #cat maxpool_fea and avgpool_fea 
        lane_fea = self.lane_feature_dropout(lane_fea) #lane_fea

        obs_fea = self.obs_feature_fc(obs_fea) #2 MLP

        tot_fea = torch.cat([lane_fea, obs_fea], 1)# lane_fea + obs_fea cat
        out_c = self.classify(tot_fea) #cls
        out_r = self.regress(torch.cat([tot_fea, out_c], 1)) #reg
