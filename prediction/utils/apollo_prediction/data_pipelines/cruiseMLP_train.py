...
def save_FCNN_CNN1D(model, filename): #CruiseModelParameter
...
def load_data(filename):
    ...
    return samples['data']  #sample_dict
...
def data_preprocessing(data):
    ...
    32:68               x3
    68:-dim_out-8       x4
    -dim_out-8:-dim_out x5   surround_obs
    -dim_out:           Y
    Preprocess the data.
    (Only for non data-loader case)
    ...
    X_obs_old_features = data[:, 0:23]#X对象的旧特征  障碍物在车道上的坐标值及变化情况/速度/加速度/偏航角/到车道左右边线距离和变化情况
    X_surround_obs = data[:, -dim_output-8:-dim_output]#X周边对象特征 8个
    X_obs_now = data[:, 23:32]#X对象1个新特征  9个
    X_obs_hist_5 = data[:, 23:68] #X对象5个历史特征 45=5*9
    X_lane = data[:, 68:-dim_output-8]#X道路特征   4*20


    # mask out those that don't have any history
    # mask5 = (data[:,53] != 100)

    X = np.concatenate((X_obs_old_features, X_obs_hist_5, X_lane), axis=1)  #old_feature, hist_5,lane
    # X = X[mask5, :]
    y = data[:, -dim_output:]  #label output_dim
    # y = y[mask5, :]

    # Binary classification  Binary {0,1}, if x>0: 1?0
    y[:, 0] = (y[:, 0] > 0).astype(float)
    #y[:, 0] = np.logical_and((y[:, 0] > 0), (y[:, 1] < 1.0))

    # Random shuffling
    X_new, X_dummy, y_new, y_dummy = train_test_split(
        X, y, test_size=0.0, random_state=233)  #random split
...
def train_vanilla(train_X, train_y, model, optimizer, epoch, batch_size=2048, balance=1.0):
    ...
    for i in range(num_of_batch):
        ...
        c_pred, r_pred = model(X)  #train
        loss = loss_fn(c_pred, r_pred, y, balance)  #loss
...
if __name__ == "__main__":
    ...
    if not args.data_loader:
        ...
        classes_train = np.asarray(train_data[:, -dim_output])#class_train
        print('Total number of training samples: {}'.format(len(classes_train)))
        print('Training set distribution:')
        print_dist(classes_train)
        classes_valid = np.asarray(valid_data[:, -dim_output])#class_valid
        print('Total number of validation samples: {}'.format(len(classes_valid)))
        print('Validation set distribution:')
        print_dist(classes_valid)

        # Data preprocessing
        X_train, y_train = data_preprocessing(train_data)  #preprocess
        X_valid, y_valid = data_preprocessing(valid_data)  #preprocess
